...
namespace DeadlockMultithreading
{
    ...
    public class AccountManager
    {
        ...
        public void Transfer()
        {
            ...
            lock (_lock1)
            {
                ...
                {
                    Console.WriteLine(Thread.CurrentThread.Name + " acquired lock on " + ((Account)_lock1).ID.ToString());
                    _fromAccount.Withdrawl(_amountToTransfer);
                    _toAccount.Deposit(_amountToTransfer);
                    Console.Write(Thread.CurrentThread.Name + " Transferred " + _amountToTransfer.ToString() + " from " +
